...
(() => {
  ...
  const INFO_CONTENT = {
    ...
    temporal_normalize: {
      ...
      body: `<p>The linear and CNN models above both look at the audio's spectral content - what frequencies are present. This is a different idea: some AI-generated audio can carry unnaturally precise, machine-regular timing underneath the music itself - a kind of rigid internal grid that's a byproduct of how the audio was generated, distinct from anything a spectral classifier looks at.</p>
             ...
             <p><strong>What this does NOT do:</strong> it does not defeat content-ID matching. Real fingerprinting systems hash <em>pairs</em> of peaks and vote on a time-offset histogram, which absorbs uniform drift and needs only a few dozen aligned hashes out of thousands. Measured against that, this stage removes about 9% of aligned hashes while the track still matches with roughly a 100x margin. Escalating does not help either: a pitch shift still matched at 50 cents (evading needs about 75, which is audibly out of tune and drifts a 3:30 track by over a second), and heavier warping and spectral dithering still matched, including all three combined. What this stage genuinely does is displace landmark timing; treat it as one small perturbation, not as protection.</p>
    ...
    tool_fix_transients: {
      ...
      body: `<p>Scans for sharp, sudden clicks or pops - the kind that come from a bad edit, a glitch, or a digital artifact, not from actual instruments. Real percussive hits (kicks, snares) have a fast but natural attack and are deliberately left alone; this only touches genuine discontinuities.</p>
             <p>When one is found, that exact moment is repaired by interpolating across it from the clean samples either side - the discontinuity is removed rather than just turned down, since simply reducing its level leaves the jump itself intact. A short taper of a few milliseconds either side keeps the repair from becoming an edge of its own; beyond that window the track is untouched.</p>
    ...
    tool_high_pass: {
      ...
      body: `<p>Removes sub-sonic rumble - content you can't hear but that still eats into your available headroom and can make a mix sound muddier than it needs to.</p>
             <p><strong>Two different frequencies matter here, and they are not the same number.</strong> The filter itself is a 30Hz high-pass. But the decision to run it at all is made on genuinely sub-sonic content: measured, a track needs energy below roughly 15Hz before this engages. A track whose lowest content is at 20Hz or above is left completely alone - not "barely filtered", but bit-for-bit identical.</p>
             <p><strong>When it does engage, it is not inaudible.</strong> A 30Hz high-pass attenuates 30Hz by about 6dB and 40Hz by about 0.8dB, and those are audible frequencies on a system that can reproduce them. That is the intended trade: it only runs when the track has rumble worth removing, and removing it costs a little of the very bottom end.</p>
             <p>It runs early, before any stage that measures level, so rumble cannot inflate the readings the compressor and limiter act on.</p>`,
    },
    tool_fix_phase: {
      ...
      body: `<p>Checks whether the left and right channels are working together or fighting each other (see the Stereo correlation explanation for the actual measurement). If content is meaningfully out of phase, it can partially cancel out when the track is played back in mono - this step corrects that so the track stays safe on any playback system.</p>
             ...
             <p><strong>Two targeted things, both automatic:</strong></p>
             <p><strong>1. Bass to mono below 120Hz.</strong> Standard practice for pop and club. You can't localise bass by ear anyway, that band carries most of a mix's energy, and out-of-phase low end is exactly what cancels when a track is summed to mono — or what makes a record hard to cut to vinyl. Where the two sides genuinely oppose each other, one is flipped before summing so the energy adds rather than cancelling to nothing.</p>
             <p><strong>2. Phase repair between 120 and 300Hz</strong>, only if that band is actually out of phase. Mono compatibility is a low-end problem in practice, so the correction stays there and your stereo image above it is untouched.</p>
    ...
    tool_saturate: {
      ...
      body: `<p>Runs the signal through a tanh curve, which rounds off the tops of the loudest peaks and generates odd-order harmonics in the process. This is the classic console/transformer/tape effect, at mastering-bus strength. <strong>Off by default.</strong></p>
             ...
             <p><strong>What it does NOT do:</strong> this is not an EQ. The largest octave-band change measured on real tracks is 0.10dB at Light, 0.21dB at Medium and 0.56dB at Strong. The audible effect is peak density. If you are after a tonal change, this is not the tool.</p>
    ...
    tool_multiband_compress: {
      ...
      body: `<p>Gently compresses four frequency bands independently, rather than the whole signal at once - a light touch aimed at smoothing out any band that's poking out too far relative to the others, for a more balanced overall tone. Deliberately conservative, not a loudness-maximizing effect.</p>
             <p><strong>The bands are split at 100Hz, 800Hz and 5kHz.</strong> The 800Hz-5kHz band matters most: that is where vocal presence, snare crack and guitar bite live. An earlier 3-band version put everything above 2kHz together, so a loud cymbal or an "s" would duck the lead vocal's presence along with it. Now they have separate controls.</p>
             <p><strong>Each band has its own attack and release</strong> - slow for the low end (30ms attack, 200ms release: a 60Hz cycle is 16ms long, so reacting faster than that would track the waveform rather than the level) and fast up top (3ms/60ms). Without time constants a compressor cannot let a transient through, which is what makes drums lose their punch.</p>
             <p>The bands are split so that they sum back to exactly the original signal. The previous filters did not, which added a small level bump at each crossover.</p>
  ...
  const TOOLS = [
    ...
    { id: "fix_transients", group: "chainGroupMaster", name: "Surgical transient/pop fix", desc: "Finds genuine clicks and pops and bridges across just that moment, with a short taper of a few milliseconds either side so the repair does not itself become an edge. Skips sustained bursts like vocal consonants, and its post-chain re-check only corrects anomalies this chain introduced — never sharp edges that were already in your source.", info: "tool_fix_transients" },
    ...
    { id: "high_pass", group: "chainGroupCleanup", name: "High-pass filter", desc: "Filters at 30Hz, but only engages when the track has genuine sub-sonic rumble \u2014 measured below about 15Hz. A clean track is left bit-for-bit untouched. Runs early, before anything measures level, so rumble can't inflate the readings the later stages act on.", info: "tool_high_pass" },
